...
output_dir = "../data/sequences/flow_seq_mirai_perdevice_v2"
...
def compute_device_feature_matrix(combined):
    ...
    ONCE - vectorized, no per-window rebuilding. Returns (n_rows, 19) array.
    """
    n = len(combined)
    ...
    # sliding-window unique-destination count (fan-out signal, high = scanning)
    # AND sliding-window max-repeat-count (repeat-contact signal, high = C2 beaconing)
    ...
    return np.concatenate([numeric_vals, conn_onehot, port_flag,
                            ...
                            repeat_counts.reshape(-1, 1)], axis=1)
